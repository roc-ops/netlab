...
* *netlab* validates ArcOS with the **stock `netlab --validate` framework** over the same docker-exec + `confd_cli | display json` path used to deploy config (no gNMI/SSH/NETCONF, no netlab-core changes) -- OSPFv2, BGP, IS-IS and static routing all pass a fresh-boot `--validate`. See [](install/arcos.md) for the mechanism, the validated-module matrix, and the virtual-image gaps below.
...
* VRRP / first-hop-redundancy (the `gateway` module) config renders and commits, but the virtual container image cannot exercise VRRP master/backup election or the virtual-IP datapath, so no `--validate` check is shipped for it -- documented gap, not a functional one.
