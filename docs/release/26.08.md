...
* **netlab status --instance** reports the [observed interface and link state](netlab-status-interfaces) of a containerlab node in its JSON or YAML output.
...
* **netlab api** jobs that hit a fatal error are no longer stuck in the 'running' state
* **netlab api** status requests no longer change the working directory of a running job
