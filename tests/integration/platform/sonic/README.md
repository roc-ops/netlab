...
| `10-vrrp-version.yml` | gateway, vrrp.version plugin | VRRP pinned to v2 -- **re-verified live 2026-08-29 on the converged device**: `show vrrp` reports `Protocol Version 2`. The previous **"Master state confirmed" claim is WRONG** and has been removed: VRRP sits in `Initialize` on both nodes, and identically so on the retired `sonic_clab` device, so it is a stale claim rather than a regression. **Narrowed, not solved**: the kernel side is entirely healthy -- `vrrp-0-1` is an UP/LOWER_UP macvlan on `Ethernet0` with the VRRP MAC `00:00:5e:00:01:01` and the VIP `172.31.43.254/24`, the parent carries `172.31.43.1/24`, and `vrrpd=yes` with the daemon running. So this is NOT the intfmgrd-overwrites-kernel-state class these templates already work around twice (routed-port IPs, VLAN members) -- it is inside vrrpd. Chasing it further is out of scope for the device convergence that found it. (Template lives in `netsim/extra/vrrp/version/sonic-clab.j2`, NOT `netsim/ansible/templates/` -- it's a node_config plugin, different search path.) |
